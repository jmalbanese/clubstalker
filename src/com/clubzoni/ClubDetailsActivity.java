...
public class ClubDetailsActivity 	extends DetailsCommon
{
...
//    private EscortIndexAdapter adapter;
    
    public ClubDetailsActivity()
    {
    }
    ...
    public void onCreate(Bundle savedInstanceState) 
    {
		...
		setContentView(R.layout.clubdetailsactivity);
...
*/
	...
	private void postToDisplay()
	{
		...
		if( entity != null)
		{
			...
			mel = entity.getMediaElements().findByType(Constants.MEDIAELEMENT_TYPE_MAIN);
			if( mel == null)
			{
				mel = entity.getMediaElements().findByType(Constants.MEDIAELEMENT_TYPE_FULL);
			}
			...
			AppCommon.getInstance().getImageLoader().DisplayImage(pixuri, (ImageView) findViewById(R.id.image), Constants.DETAIL_LANDSCAPE_SIZE);	
	...
	private void initControls()
	{
		...
		buttonVideo.setOnClickListener( new View.OnClickListener()
		{
			...
			{
				onVideoRequested(); // some devices don't have a search buttonnew
			}
		});

		if(entity.getMediaElements().findByType(Constants.MEDIAELEMENT_TYPE_VIDEO)==null)
		{
			buttonVideo.setEnabled(false);
		}

		
		// the gallery button - do we have full images for the girl? we have turned off thumbnail view, it's boring
		...
		if( entity.getMediaElements().findByType(Constants.MEDIAELEMENT_TYPE_FULL) == null)
		{
			buttonGallery.setEnabled(false);
		}
		else		// we have gallery - let the image click too
		{
			// image
...
// chat? need a server side - defer for now 
		...
		{	
			public void onClick(View v)
			{
				externalIntent.onChatRequested();
			}
		});
*/
		// email
		...
		if(!entity.getEnable_address().equals(Constants.ENABLED))
		{
