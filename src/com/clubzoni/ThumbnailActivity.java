...
import com.fedorvlasov.lazylist.ThumbnailAdapter;
	...
	private void handleIntent(Intent intent)
	{
		...
		if(extras != null)
		{
			...
			entity = appcommonInstance.getVenues().findByUuid(uuid);
	       	thumbnails = entity.getMediaElements().getAllByType(Constants.MEDIAELEMENT_TYPE_THUMB);
	        adapter=new ThumbnailAdapter(this, thumbnails);
	        thumbnailView =(GridView)findViewById(R.id.thumbnailview);
	        thumbnailView.setAdapter(adapter);
	        thumbnailView.setOnItemClickListener(new OnItemClickListener() {
	            public void onItemClick(AdapterView<?> parent, View v, int position, long id) 
	            {
	    	        Intent intent = new Intent();
	    	        intent.putExtra(Constants.ENTITY_TYPE_UUID, entity.getUuid());	// what entity
	    	        intent.setClass(com.clubzoni.ThumbnailActivity.this,PortraitActivity.class);
